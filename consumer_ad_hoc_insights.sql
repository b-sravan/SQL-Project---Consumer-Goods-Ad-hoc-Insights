...
SELECT CASE 
 ...
 concat(round(sum(sold_quantity)/1000000,2),"M") 
 as total_sold_quanity_in_millions FROM temp_table
...
#Task # 10
WITH temp_table AS (
    select p.division, s.product_code,  p.product , sum(sold_quantity) 
    AS total_sold_quantity,
    rank() OVER (partition by division order by 
